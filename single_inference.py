...
CHECKPOINT         = r"C:/Users/Rikon/Documents/GitHub/zero-latency/exports/model_fp32.pt"
DATAROOT           = r"C:/Users/Rikon/zero-latency/data/raw/nuscenes"
VERSION            = "v1.0-mini"
MIN_TOTAL_MOVEMENT = 5.0    # metres — skip static scenes below this
MAX_SEARCH         = 200    # max samples to scan
SEED               = None   # set to int for reproducibility e.g. SEED = 54

OUTPUT_PNG         = "multi_agent_prediction.png"
OUTPUT_MP4         = "multi_agent_prediction.mp4"

# Animation settings
# Playback is slowed to 0.25× by repeating each data-frame 4 times via
# repeat_delay logic inside _update, while keeping the output FPS at 20
# so media players stay smooth. Effective trajectory speed = 5 steps/sec.
ANIM_FPS           = 20     # output MP4 frame rate (keep at 20 for smooth playback)
ANIM_SPEED_MULT    = 4      # repeat each logical step N times → 0.25× speed
ANIM_DPI           = 120    # dots per inch for the rendered frames
ANIM_BITRATE       = 1800   # kbps — increase for higher quality
ANIM_END_HOLD_SEC  = 3      # seconds to hold on the final complete frame
...
class TrajectoryPredictor(torch.nn.Module):
    ...
    def forward(self, x, positions, map_features, agent_types=None):
...
print(f"\nLoading dataset: {VERSION} from {DATAROOT}")
...
def find_random_moving_sample(dataset, min_movement, max_search, seed):
    ...
    for i, idx in enumerate(indices[:max_search]):
        ...
        if scene_has_movement(s, min_movement):
            ...
            return s, idx
    raise RuntimeError(f"No moving scene found in {max_search} samples.")
...
TYPE_ICONS  = {0: "[V]", 1: "[P]", 2: "[C]"}   # ASCII — DejaVu Sans lacks emoji glyphs
TYPE_COLORS = {0: "#4e79a7", 1: "#f28e2b", 2: "#59a14f"}  # blue / orange / green

raw_types   = sample["x"][-1, :, 7].round().long()        # (max_agents,) from last past step
agent_types_np  = raw_types[:num_agents].numpy().astype(int)
agent_types_tensor = raw_types[:num_agents].unsqueeze(0).to(device)  # (1, N) for model
...
for agent_id in range(num_agents):
    ...
    print(
        f"  Agent {agent_id:>2} {TYPE_ICONS.get(int(agent_types_np[agent_id]),'?')}"
        f" [{TYPE_NAMES.get(int(agent_types_np[agent_id]),'unknown')}]"
...
def _make_title(extra=""):
    return (
        f"Multi-Agent Trajectory Prediction  |  Sample idx={sample_idx}\n"
...
# Figure dimensions — match data aspect ratio; add right margin for legend
...
def _apply_shared_axes(ax):
    """Apply identical view limits, labels, and grid to any Axes object."""
...
def _add_outside_legend(fig, ax):
    """Place legend in a dedicated margin to the right of the axes."""
    legend = ax.legend(
        handles=_build_legend_handles(),
        loc="upper left",
        bbox_to_anchor=(1.01, 1.0),   # anchor just outside the right axes edge
        borderaxespad=0,
        fontsize=7.5,
        framealpha=0.95,
        edgecolor="#cccccc",
...
def _draw_static_frame(ax):
    """Plot all agents — past, all modes, best prediction, ground truth."""
    for agent_id in range(num_agents):
        color  = colors[agent_id % len(colors)]
        ...
        # ADE/FDE annotation next to the last predicted point
        ax.annotate(
            f"A{agent_id}",
...
# Same margin fractions as the animation figure for visual consistency.
# bbox_inches="tight" on savefig handles any remaining overhang.
...
# Subtitle row: legend reading guide
fig_static.text(
    0.01, 0.005,
...
fig_static.savefig(OUTPUT_PNG, dpi=150, bbox_inches="tight")
plt.close(fig_static)
print(f"\nStatic plot saved → {OUTPUT_PNG}")
...
# ---------------------------------------------------------------------------
# Frame layout:
#   frames 0 .. T_past-1            : past trajectory grows incrementally
#   frames T_past .. T_past+T_future-1 : prediction + GT grow step by step
#   end hold (ANIM_END_HOLD_SEC s)  : hold on the complete frame
#
# Each logical step is rendered ANIM_SPEED_MULT times → 0.25× speed while
# the output FPS stays smooth at ANIM_FPS.
# ---------------------------------------------------------------------------
...
# Reserve right margin so the legend is inside the figure canvas — this is the
# correct approach for Animation.save(), which does not accept bbox_inches.
# LEGEND_W / fig_w gives the fraction of the figure the legend panel occupies.
_legend_frac = LEGEND_W / fig_w
...
def _update(frame):
    """Animation update: map render frame → logical step, update all artists."""
    ...
    # Collect all updated artists for blit
...
ani = animation.FuncAnimation(
    ...
    blit=False,   # blit=True can prevent the title updating on some backends
)

# FFmpeg writer — requires ffmpeg on PATH
try:
    ...
    ani.save(OUTPUT_MP4, writer=writer, dpi=ANIM_DPI)
...
print(f"\nTo reproduce this scene: set SEED = {sample_idx}")
print("Done.")
