...
def build_optimizer(
    ...
    # Combine model parameters and loss module parameters
    params = [
...
def create_checkpoint(
    model: nn.Module,
    criterion: nn.Module, # <-- Add criterion
    ...
    return {
        ...
        "criterion": criterion.state_dict(), # <-- Save criterion state
...
def load_resume_checkpoint(
    ...
    for resume_path in candidate_paths:
        ...
        try:
            ...
            #
        except Exception as exc: # <-- This is what your IDE was looking for!
...
def train_one_epoch(
   model_forward: nn.Module,
    ...
    for step, batch in enumerate(loader, start=1):
        ...
        # 1. The TRY block must wrap the forward AND backward pass to catch OOMs
        try:
            # 2. AUTOCAST ONLY wraps the forward pass and loss computation
            with torch.autocast(device_type=device.type, dtype=amp_dtype, enabled=use_autocast):
                ...
                # Using the new AutoTuned loss module
            ...
            # 3. BACKWARD pass is inside the 'try', but outside the 'autocast'
            if use_scaler:
                scaler.scale(scaled_loss).backward()
            else:
                scaled_loss.backward()

        # 4. OOM Handling catches anything in the 'try' block
            ...
            continue  # IDE will be happy now!

        # 5. OPTIMIZER STEP (Runs only if the try block succeeds)
...
def evaluate(
    ...
    for step, batch in enumerate(loader, start=1):
        ...
        # USE THE MODULE INSTEAD OF THE FUNCTION
          traj_loss, ade, fde = criterion(traj, gt, return_metrics=True) 
